...
#if 0
	dev_info(dcp->dev, "---> context %u, offset %u, depth %u\n", context, offset, depth);
...
static bool dcpep_cb_map_piodma(struct apple_dcp *dcp, void *out, void *in)
{
	...
	resp->dva = dma_map_resource(dcp->piodma, map->paddr, map->size,
				     DMA_BIDIRECTIONAL, 0);
...
static bool dcpep_cb_allocate_buffer(struct apple_dcp *dcp, void *out, void *in)
{
	...
	dev_info(dcp->dev, "allocated %llx bytes to (%u, %llx, %llx)\n",
		 resp->dva_size, resp->mem_desc_id, resp->paddr, resp->dva);
...
{
	dcp_push(dcp, DCP_CONTEXT_CB, LATE_INIT_SIGNAL, 0, 4, NULL,
		 boot_5, NULL);
}
...
{
	dcp_push(dcp, DCP_CONTEXT_CB, CREATE_DEFAULT_FB, 0, sizeof(u32), NULL, boot_2, NULL);
}

static bool dcpep_cb_boot_1(struct apple_dcp *dcp, void *out, void *in)
{
	dcp_push(dcp, DCP_CONTEXT_CB, SET_CREATE_DFB, 0, 0, NULL, boot_1_5, NULL);
	return false;
}
...
static void dcp_set_4k(struct apple_dcp *dcp, void *out, void *cookie)
{
	...
	dcp_push(dcp, DCP_CONTEXT_CMD, SET_DIGITAL_OUT_MODE, sizeof(req),
		 sizeof(u32), &req, modeset_done, NULL);
}

void dcp_modeset(struct platform_device *pdev) {
	struct apple_dcp *dcp = platform_get_drvdata(pdev);
	u32 handle = 2;

	dcp_push(dcp, DCP_CONTEXT_CMD, SET_DISPLAY_DEVICE, sizeof(handle),
