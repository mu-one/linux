...
static int apple_pcie_setup_refclk(volatile void __iomem *rc, volatile void __iomem *port, unsigned idx)
{
	u32 stat;
	int res;
	int count = 0;

	printk("setting up refclk %p, %p, %u", rc, port, idx);
	res = readl_poll_timeout(rc + CORE_RC_PHYIF_STAT, stat, (stat & CORE_RC_PHYIF_STAT_REFCLK), 100, 50000);
	if (res < 0) {
		printk("%s: core refclk wait timed out.\n", __func__);
		return res;
	}
	printk("%u\n", count++);
	...
	printk("%u\n", count++);
	res = readl_poll_timeout(rc + CORE_LANE_CFG(idx), stat, (stat & CORE_LANE_CFG_REFCLK0ACK), 100, 50000);
	printk("%u\n", count++);
	if (res < 0) {
		printk("%s: lane refclk%d wait timed out (port %d).\n", __func__, 0, idx);
		return res;
	}
	rmwl(0, CORE_LANE_CFG_REFCLK1, rc + CORE_LANE_CFG(idx));
	printk("%u\n", count++);
	res = readl_poll_timeout(rc + CORE_LANE_CFG(idx), stat, (stat & CORE_LANE_CFG_REFCLK1), 100, 50000);
	printk("%u\n", count++);
	if(res < 0) {
		printk("%s: lane refclk%d wait timed out (port %d).\n", __func__, 1, idx);
		return res;
...
static int apple_msi_init(struct apple_pcie *pcie)
{
	...
	rc = devm_of_iomap(pcie->dev, to_of_node(fwnode), 1, NULL);
	printk("Trying to base %p\n", rc);
	printk("base %X\n", readl(rc));
	printk("cfgacc %X\n", readl(rc + CORE_LANE_CTL(0)));
	printk("cfgacced\n");

	power = devm_gpiod_get_index(pcie->dev, "pwren", 0, 0);
	if (IS_ERR(power)) {
		dev_err(pcie->dev, "failed to get power 0 gpio: %ld\n", PTR_ERR(power));
		return PTR_ERR(power);
	}

	printk("Enumerating ports\n");
	for (i = 0; i < NUM_PORTS; ++i) {
		int res;
		struct gpio_desc *reset;
		uint32_t linksts, stat;

		port = devm_of_iomap(pcie->dev, to_of_node(fwnode), i + 3, NULL);

		if (IS_ERR(port)) {
			dev_err(pcie->dev, "Failed to map port %u\n", i);
			return -ENODEV;
		}

                reset = devm_gpiod_get_index(pcie->dev, "reset", i, 0);
                if (IS_ERR(reset)) {
			dev_err(pcie->dev, "failed to get reset #%d gpio: %ld\n", i, PTR_ERR(reset));
                        return PTR_ERR(reset);
                }

		linksts = readl(port + PORT_LINKSTS);
		dev_err(pcie->dev, "Port %u link status: %s, %s (%X)", 
				i,
				(linksts & PORT_LINKSTS_UP) ? "up" : "down",
				(linksts & PORT_LINKSTS_BUSY) ? "busy" : "idle",
				linksts);
		...
		res = apple_pcie_setup_refclk(rc, port, i);
		if(res < 0) {
			dev_err(pcie->dev, "refclk setup failed");
			return res;
		}
		...
		res = readl_poll_timeout(port + PORT_STATUS, stat, (stat & PORT_STATUS_READY), 100, 250000);
		if (res < 0) {
			dev_err(pcie->dev, "status ready wait timed out (port %u).\n", i);
			return res;
		}
		...
		res = readl_poll_timeout(port + PORT_LINKSTS, stat, !(stat & PORT_LINKSTS_BUSY), 100, 250000);
		if(res < 0) {
			dev_err(pcie->dev, "link not busy wait timed out (port %u).\n", i);
			return res;
		}

		writel(0xfb512fff, port + PORT_INTMSKSET);
		writel(0x04aed000, port + PORT_INTSTAT);

		usleep_range(5000, 10000);

		rmwl(0, PORT_LTSSMCTL_START, port + PORT_LTSSMCTL);
		res = readl_poll_timeout(port + PORT_LINKSTS, stat, (stat & PORT_LINKSTS_UP), 100, 500000);
		if (res < 0) {
			dev_err(pcie->dev, "link up wait timed out (port %u).\n", i);
			return res;
		}
		...
		writel((5 << PORT_MSICFG_L2MSINUM_SHIFT) | PORT_MSICFG_EN, port + PORT_MSICFG);
	}

	ret = of_property_read_u32_index(to_of_node(fwnode), "msi-interrupts",
					 0, &pcie->msi_base);
	if (ret) {
		dev_err(pcie->dev, "missing msi-ranges 0");
		return ret;
	}
	...
	ret = of_property_read_u32_index(to_of_node(fwnode), "msi-interrupts",
					 1, &pcie->nvecs);
	if (ret) {
		dev_err(pcie->dev, "missing msi-ranges 1");
		return ret;
	}
