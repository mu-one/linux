...
struct apple_m1_spmi_data {
	...
	u32 tx[MSG_SIZE/4];
	unsigned int txrp, txmax;
	u32 rx[MSG_SIZE/4];
	u32 ack[MSG_SIZE/32];
	unsigned int ackwp;
	unsigned int rxwp, rxmax;
};
...
static void apple_m1_spmi_continue(struct apple_m1_spmi_data *spmi)
{
	...
	unsigned int step = min(spmi->bytes, 8u);
	WARN_ON(step < 1);
	...
	u16 addr, u8 *rxbuf, size_t rxlen, const u8 *txbuf, size_t txlen)
{
	...
	if (!timeout) {
		...
		if (ret) {
			dev_warn(spmi->dev, "SPMI command [%02x,%02x,%04x,%zu/%zu] returned [%08x,%08x]\n",
				 opc, sid, addr, rxlen, txlen,
...
static void apple_m1_spmi_chained_irq(struct irq_desc *desc)
{
	...
	unsigned int i, b, irq;
	u32 stat[IRQ_REGS + IRQ_LOCAL_REGS], s, val;

	chained_irq_enter(chip, desc);

	for (i = 0; i < IRQ_REGS + IRQ_LOCAL_REGS; i++) {
		stat[i] = readl(spmi->base + REG_IRQSTAT(i)) &
			  readl(spmi->base + REG_IRQEN(i));
	}
	...
	for (i = 0; i < IRQ_REGS; i++) {
		...
		while (s) {
			...
			writel(val & ~BIT(b), spmi->base + REG_IRQEN(i));
			spin_unlock(&spmi->lock);

			writel(BIT(b), spmi->base + REG_IRQSTAT(i));
...
static void apple_m1_spmi_irq_ack(struct irq_data *d)
{
	...
	writel(BIT(irq & 31), spmi->base + REG_IRQSTAT(irq >> 5));
}
...
static int apple_m1_spmi_irq_domain_translate(struct irq_domain *domain,
...
static int apple_m1_spmi_irq_domain_alloc(struct irq_domain *domain,
					  unsigned int virq,
...
static int apple_m1_spmi_probe(struct platform_device *pdev)
{
	...
	spmi->base = devm_platform_ioremap_resource(pdev, 0);
	if (IS_ERR(spmi->base)) {
